...
class download(object):
    ...
    def get_download_url(self):
        ...
        target_req = request.Request(
            url=self.__target_url, headers=self.__head)
        ...
        for child in download_soup.dl.children:
            if child != '\n':
                ...
                if begin_flag == True and child.a != None:
                    download_url = "http://www.biqukan.com" + \
                        child.a.get('href')
                    download_name = child.string
                    names = str(download_name).split('章')
                    name = charter.findall(names[0] + '章')
                    if name:
                        download_dict['第' + str(numbers) +
                                      '章 ' + names[1]] = download_url
                        numbers += 1
    ...
    def WriterTitle(self, name, path):
        with open(path, 'a', encoding='utf-8') as f:
            f.write(name)
            f.write('\n')

    def WiterContent(self, path, text):
        write_flag = True
        with open(path, 'a', encoding='utf-8') as f:
            for each in text:
                if each == 'h':
...
if __name__ == "__main__":
    ...
    for key, value in url_dict.items():
        d.WriterTitle(key, name)
        d.WiterContent(str(index) + name, d.Downloader(value))
